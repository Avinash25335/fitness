...
<div class="space-y-10 fade-up">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div class="flex-1 max-w-2xl">
            <form action="{{ route('blog.index') }}" method="GET" class="relative group">
                <input type="text" name="search" value="{{ request('search') }}" 
                       placeholder="Search articles, nutrition tips, workouts..." 
                       class="w-full bg-surface-2 border border-border-col rounded-2xl pl-12 pr-4 py-4 text-white focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand/30 transition-all placeholder-gray-500">
                <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-brand transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                @if(request('search'))
                    <a href="{{ route('blog.index') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-gray-500 hover:text-white uppercase font-bold tracking-widest">Clear</a>
                @endif
            </form>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('blog.index', ['search' => request('search')]) }}" 
               class="px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-widest transition-all {{ !request('category') ? 'bg-brand text-white shadow-lg shadow-brand/20' : 'bg-surface-2 text-gray-400 hover:text-white border border-border-col' }}">
                All
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('blog.index', ['category' => $cat->slug, 'search' => request('search')]) }}" 
               class="px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-widest transition-all {{ request('category') === $cat->slug ? 'bg-brand text-white shadow-lg shadow-brand/20' : 'bg-surface-2 text-gray-400 hover:text-white border border-border-col' }}">
                {{ $cat->name }}
            </a>
            @endforeach
        </div>
    ...
    <div class="relative group">
        <a href="{{ route('blog.show', $featuredPost) }}" class="flex flex-col lg:flex-row bg-gray-800 border border-gray-700 rounded-3xl overflow-hidden hover:border-brand/30 hover:shadow-2xl hover:shadow-brand/5 transition-all duration-500">
            ...
            <div class="lg:w-5/12 p-8 lg:p-12 flex flex-col justify-center">
                ...
                <h2 class="text-3xl lg:text-4xl font-black text-white mb-6 leading-tight group-hover:text-brand transition-colors">
                    {{ $featuredPost->title }}
                </h2>
                <p class="text-gray-400 leading-relaxed mb-8 line-clamp-3">
                    {{ Str::limit(strip_tags($featuredPost->content), 200) }}
                </p>
                <div class="flex items-center justify-between mt-auto pt-6 border-t border-gray-700/50">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-bold text-white">{{ $featuredPost->author->name ?? 'Admin' }}</p>
    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <a href="{{ route('blog.show', $post) }}" class="flex flex-col bg-gray-800 border border-gray-700 rounded-3xl overflow-hidden hover:border-brand/30 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand/5 transition-all duration-500 group">
                ...
                <div class="p-6 flex flex-col flex-1">
                    ...
                    <h3 class="text-xl font-bold text-white mb-3 leading-tight group-hover:text-brand transition-colors line-clamp-2">
                        {{ $post->title }}
                    </h3>
                    <p class="text-sm text-gray-500 leading-relaxed mb-6 line-clamp-2 flex-1">
                        {{ Str::limit(strip_tags($post->content), 120) }}
                    </p>
                    <div class="flex items-center justify-between pt-5 border-t border-gray-700/50 mt-auto">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-brand font-bold text-xs border border-white/10">
                                {{ strtoupper(substr($post->author->name ?? 'A', 0, 1)) }}
                            </div>
                            <span class="text-xs text-gray-400 font-bold">{{ $post->author->name ?? 'Admin' }}</span>
                        </div>
        ...
        <!-- PAGINATION -->
        <div class="mt-12 py-10 border-t border-gray-700/50 flex justify-center">
            {{ $posts->links() }}
        </div>
        @else
            <!-- EMPTY STATE -->
            @if(!$featuredPost)
            <div class="card p-20 text-center flex flex-col items-center">
                <div class="w-20 h-20 rounded-3xl bg-surface-2 border border-border-col flex items-center justify-center mb-6">
                ...
                <h3 class="text-2xl font-black text-white mb-2 tracking-tight">No articles found</h3>
                <p class="text-gray-500 mb-8 max-w-sm leading-relaxed">We couldn't find any articles matching your search or selected category.</p>
