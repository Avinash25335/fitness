...
<div class="relative fade-up" x-data="{ 
    ...
    <!-- Back Button -->
    <div class="mb-10">
        <a href="{{ route('blog.index') }}" class="group inline-flex items-center gap-2 text-gray-500 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <article class="lg:col-span-8 space-y-10">
            ...
            <header class="space-y-6">
                ...
                <div class="flex flex-wrap items-center gap-6 pt-4 border-t border-gray-700/50">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-black text-white">{{ $post->author->name ?? 'Admin' }}</p>
                            <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Scientific Contributor</p>
                        </div>
                    </div>
                    <div class="h-8 w-px bg-gray-700/50 hidden md:block"></div>
                    <div>
                        <p class="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-0.5">Published On</p>
                        <p class="text-sm font-bold text-gray-300">{{ \Carbon\Carbon::parse($post->published_at)->format('F d, Y') }}</p>
                    </div>
                </div>
            </header>
            ...
            <!-- Content -->
            <div id="article-body" class="prose prose-invert prose-brand max-w-none 
                prose-headings:font-black prose-headings:tracking-tight prose-headings:text-white
                prose-p:text-gray-400 prose-p:leading-relaxed prose-p:text-lg
                prose-li:text-gray-400 prose-strong:text-white prose-strong:font-bold
                prose-blockquote:border-l-brand prose-blockquote:bg-white/5 prose-blockquote:p-8 prose-blockquote:rounded-3xl prose-blockquote:not-italic
                space-y-8">
                {!! nl2br($post->content) !!}
            </div>

            <!-- Footer / Tags -->
            <footer class="pt-12 border-t border-gray-700/50">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div class="flex flex-wrap gap-2">
                        @foreach(['Fitness', 'Health', 'Science', 'Performance'] as $tag)
                            <span class="px-3 py-1.5 rounded-lg bg-surface-2 border border-border-col text-[10px] font-bold text-gray-500 uppercase tracking-widest hover:text-brand hover:border-brand/30 cursor-pointer transition-all">
                                #{{ $tag }}
                            </span>
                        @endforeach
                    </div>
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex gap-2">
                            @foreach(['twitter', 'facebook', 'linkedin'] as $social)
                                <button class="w-9 h-9 rounded-xl bg-surface-2 border border-border-col flex items-center justify-center text-gray-400 hover:text-white hover:border-brand/30 transition-all">
        ...
        <aside class="lg:col-span-4 space-y-8">
            <div class="sticky top-28 space-y-8">
                ...
                <!-- Table of Contents -->
                <div class="card p-8 bg-surface-2/50 backdrop-blur-xl border border-white/5">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <a href="{{ route('blog.show', $rp) }}" class="group flex gap-4 p-3 rounded-2xl bg-surface-2 hover:bg-surface-3 border border-white/5 transition-all">
                            <div class="w-20 h-20 rounded-xl overflow-hidden shrink-0 border border-white/10">
                            ...
                            <div class="flex flex-col justify-center min-w-0">
                                <h4 class="text-sm font-bold text-white group-hover:text-brand transition-colors line-clamp-2 leading-tight mb-1">{{ $rp->title }}</h4>
                ...
                <!-- Newsletter / CTA -->
                <div class="card p-8 bg-gradient-to-br from-brand/10 to-brand-dark/5 border border-brand/20 relative overflow-hidden group">
                    ...
                    <h3 class="text-lg font-black text-white mb-2 leading-tight">Master your fitness journey</h3>
                    <p class="text-xs text-gray-400 mb-6 leading-relaxed">Join 10,000+ members receiving weekly science-backed fitness tips.</p>
